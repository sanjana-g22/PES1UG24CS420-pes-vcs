...
int object_write(ObjectType type, const void *data, size_t len, ObjectID *id_out) {
    // Step 1: Build header string like "blob 16\0"
    const char *type_str;
    ...
    // +1 because we want to include the null byte '\0' at end of header
    ...
    // Step 4: Check if already exists (deduplication)
    if (object_exists(id_out)) {
        free(full);
        return 0;
    }

    // Step 5: Build the shard directory path (.pes/objects/XX/)
    char hex[HASH_HEX_SIZE + 1];
    hash_to_hex(id_out, hex);

    char shard_dir[512];
    snprintf(shard_dir, sizeof(shard_dir), "%s/%.2s", OBJECTS_DIR, hex);
    mkdir(shard_dir, 0755); // create if not exists (ignore error if exists)
